...
export default function BottomTabBar() {
    ...
    return (
        ...
        >
            ...
            >
                {/* [FIX] تقليص padding وتصغير عناصر الشريط لمنع التداخل على 375px */}
                <div className="flex items-center justify-around px-0.5 pt-1 pb-safe max-w-lg mx-auto" dir="rtl">
                    ...
                    {rightTabs.map(tab => {
                        ...
                        const Icon = tab.icon;
                        return (
                            <Link key={tab.href} href={tab.href} className="flex-1 flex flex-col items-center gap-[2px] py-1.5 min-w-0" prefetch>
                                <motion.div whileTap={{ scale: 0.85 }} className="flex flex-col items-center gap-[2px]">
                                    <div className={`relative flex items-center justify-center w-6 h-6 rounded-xl transition-all duration-300 ${active ? 'bg-[#C9A96E]/20' : ''}`}>
                                        {active && (
                                            <motion.div layoutId="activeTabRight" className="absolute inset-0 rounded-xl bg-[#C9A96E]/15" transition={{ type: 'spring', damping: 25 }} />
                                        )}
                                        <Icon
                                            className={`w-[15px] h-[15px] relative z-10 transition-all duration-300 ${active ? 'text-[#C9A96E]' : 'text-white/40'}`}
                                            strokeWidth={active ? 2.5 : 1.8}
                                        />
                                    </div>
                                    {/* [FIX] نص مضغوط مع truncate لمنع التكسر */}
                                    <span className={`text-[8px] font-black tracking-wide leading-none transition-colors truncate max-w-[40px] ${active ? 'text-[#C9A96E]' : 'text-white/35'}`}>
                    ...
                    <div className="flex-none flex flex-col items-center relative -mt-5 gap-0.5 px-0.5">
                        <motion.button
                            whileTap={{ scale: 0.88 }}
                            whileHover={{ scale: 1.06 }}
                            onClick={handleWhatsApp}
                            className="w-[46px] h-[46px] rounded-full flex items-center justify-center relative border-[2.5px] border-white shadow-xl"
                            style={{
                        ...
                        <span className="text-[7px] font-black text-[#25D366] tracking-widest">WA</span>
                    </div>
                        ...
                        return (
                            <Link key={tab.href} href={tab.href} className="flex-1 flex flex-col items-center gap-[2px] py-1.5 min-w-0" prefetch>
                                <motion.div whileTap={{ scale: 0.85 }} className="flex flex-col items-center gap-[2px]">
                                    <div className={`relative flex items-center justify-center w-6 h-6 rounded-xl transition-all duration-300 ${active ? 'bg-[#C9A96E]/20' : ''}`}>
                                        {active && (
                                            <motion.div layoutId="activeTabAuction" className="absolute inset-0 rounded-xl bg-[#C9A96E]/15" transition={{ type: 'spring', damping: 25 }} />
                                        )}
                                        <Icon
                                            className={`w-[15px] h-[15px] relative z-10 transition-all duration-300 ${active ? 'text-[#C9A96E]' : 'text-white/40'}`}
                                            strokeWidth={active ? 2.5 : 1.8}
                    ...
                    {/* ─── 4. مبدل العملة (أيقونة فقط بدون نص مطوّل) ─── */}
                    <motion.button
                        whileTap={{ scale: 0.82 }}
                        onClick={handleCycleCurrency}
                        className="flex-1 flex flex-col items-center gap-[2px] py-1.5 min-w-0"
                        title={isRTL ? 'اضغط لتبديل العملة' : 'Tap to switch currency'}
                    >
                        <div className="relative flex items-center justify-center w-6 h-6 rounded-xl transition-all duration-300">
                            <span
                                className="w-[18px] h-[18px] rounded-full flex items-center justify-center text-[8px] font-black relative z-10 shadow-md transition-all"
                                style={{ background: currentCurrency.color, color: 'white' }}
                            >
                                {currentCurrency.symbol}
                            </span>
                        </div>
                        {/* [FIX] عرض كود العملة مضغوطاً بدون نص إضافي */}
                        <span className="text-[8px] font-black tracking-wide leading-none text-[#C9A96E] truncate max-w-[40px]">
                        ...
                        return (
                            <Link key={tab.href} href={tab.href} className="flex-1 flex flex-col items-center gap-[2px] py-1.5 min-w-0" prefetch>
                                <motion.div whileTap={{ scale: 0.85 }} className="flex flex-col items-center gap-[2px]">
                                    <div className={`relative flex items-center justify-center w-6 h-6 rounded-xl transition-all duration-300 ${active ? 'bg-[#C9A96E]/20' : ''}`}>
                                        {active && (
                                            <motion.div layoutId="activeTabUser" className="absolute inset-0 rounded-xl bg-[#C9A96E]/15" transition={{ type: 'spring', damping: 25 }} />
                                        )}
                                        <Icon
                                            className={`w-[15px] h-[15px] relative z-10 transition-all duration-300 ${active ? 'text-[#C9A96E]' : 'text-white/40'}`}
                                            strokeWidth={active ? 2.5 : 1.8}
                                    ...
                                    {/* [FIX] تقليص نص "حسابي" لـ "حسابي" ونص "دخول" لـ "دخول" بدون truncate إضافي */}
                                    <span className={`text-[8px] font-black tracking-wide leading-none transition-colors truncate max-w-[40px] ${active ? 'text-[#C9A96E]' : 'text-white/35'}`}>
                                        {isRTL ? (isLoggedIn ? 'حسابي' : 'دخول') : (isLoggedIn ? 'Me' : 'Login')}
                                    </span>
                                </motion.div>
                            </Link>
                        );
