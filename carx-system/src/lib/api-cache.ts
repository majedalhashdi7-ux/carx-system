/**
 * نظام ذاكرة التخزين المؤقت للعميل (Client-side API Cache) - CAR X
...
export const apiCache = {
    set: (key: string, data: unknown, ttl = DEFAULT_TTL) => {
        ...
        if (typeof window !== 'undefined') {
            try {
                sessionStorage.setItem(`carx_api_cache_${key}`, JSON.stringify({ data, timestamp: expiresAt }));
            } catch {}
        }
    },
    ...
    get: (key: string) => {
        ...
        if (typeof window !== 'undefined') {
            try {
                const stored = sessionStorage.getItem(`carx_api_cache_${key}`);
                if (stored) {
                    ...
                    if (Date.now() <= parsed.timestamp) {
                        ...
                        sessionStorage.removeItem(`carx_api_cache_${key}`);
                    }
                }
            } catch {}
        }
        ...
        if (typeof window !== 'undefined') {
            try {
                Object.keys(sessionStorage).forEach(k => {
                    if (k.startsWith('carx_api_cache_')) sessionStorage.removeItem(k);
                });
            } catch {}
        }
    }
