...
class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error('⚠️ [CRITICAL ERROR] Caught by CAR X ErrorBoundary:', error, errorInfo);
    // رفع الخطأ تلقائياً إلى Sentry مع سياق المكون
...
/**
 * تصميم مودرن وفخم لواجهة الخطأ في معرض CAR X
 */
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black text-white p-6 selection:bg-red-500/30">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-luxury-gold/5 blur-[120px] rounded-full animate-pulse" style={{ animationDelay: '1s' }} />
      </div>
      ...
      >
        ...
        <h2 className="text-3xl font-black mb-4 tracking-tight uppercase italic text-red-500">
          انقطاع في النظام
        </h2>
        
        <p className="text-white/60 mb-8 leading-relaxed text-sm">
          نعتذر، حدث خطأ غير متوقع في معالجة طلبك الحالي. <br />
          فريق التطوير الفني لـ CAR X يعمل على استقرار المنصة.
        </p>
          ...
          <div className="mb-8 text-right">
            <details className="group">
              <summary className="cursor-pointer text-xs font-bold text-white/30 uppercase tracking-[0.2em] group-hover:text-white/60 transition-colors list-none flex items-center gap-2">
                ...
                التفاصيل التقنية (للإدارة)
              </summary>
              <div className="mt-4 p-4 rounded-2xl bg-black/40 border border-white/5 font-mono text-[10px] text-red-400/80 overflow-x-auto text-left" dir="ltr">
        ...
        <div className="grid grid-cols-2 gap-4">
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={resetError}
            className="flex items-center justify-center gap-3 px-6 py-4 bg-luxury-gold text-black rounded-2xl font-black text-sm uppercase tracking-wider hover:bg-white transition-all shadow-[0_0_20px_rgba(212,175,55,0.2)]"
          >
