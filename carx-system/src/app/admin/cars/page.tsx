...
function KoreanImportButton({ onImported }: { onImported: () => void }) {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      setStatus('success');
      setMsg(`✅ تم استيراد ${imported} سيارة${skipped > 0 ? ` · تجاوز ${skipped} مكرر` : ''}`);
      localStorage.setItem(STORAGE_KEY, String(Date.now()));
      setCooldown(Math.ceil(COOLDOWN_MS / 1000));
      onImported();
      setTimeout(() => setStatus('idle'), 8000);
    } catch (e: any) {
      setStatus('error'); setMsg(e.message || 'فشل الاستيراد');
  ...
  return (
    <div className="flex flex-col gap-2">
      <motion.button
        ...
        disabled={isDisabled}
        className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all relative overflow-hidden
          ${isDisabled
            ? 'bg-white/5 border border-white/10 text-white/30 cursor-not-allowed'
            : 'bg-luxury-gold text-black hover:bg-white shadow-[0_0_20px_rgba(212,175,55,0.2)] cursor-pointer'
          }`}
      >
        ...
        {status === 'loading' ? 'جاري الاستيراد...'
          ...
          : 'جلب سيارات كوريا'}
        {cooldown > 0 && (
          <div className="absolute bottom-0 left-0 h-0.5 bg-luxury-gold/40 transition-all"
      ...
      <AnimatePresence>
        {msg && (
          <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className={`flex items-start gap-2 px-3 py-2 rounded-lg text-xs font-bold
              ${status === 'success' ? 'bg-green-500/10 border border-green-500/20 text-green-400'
                : 'bg-red-500/10 border border-red-500/20 text-red-400'}`}
          >
            <AlertCircle className="w-3.5 h-3.5 mt-0.5 shrink-0" />{msg}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
export default function AdminCarsPage() {
  ...
  const [filter, setFilter]       = useState<'all' | 'korean_import' | 'hm_local'>('all');
  ...
  const fetchCars = useCallback(async () => {
    ...
    } catch { setError('فشل جلب السيارات من الخادم'); }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (!res.error) fetchCars(); else alert(res.error || 'فشل الحذف');
    } catch { alert('فشل الاتصال بالخادم'); }
  };

  useEffect(() => { fetchCars(); }, [fetchCars]);

  const koreanCount = cars.filter(c => c.source === 'korean_import' || c.source === 'encar_korea').length;
  const localCount  = cars.filter(c => c.source !== 'korean_import' && c.source !== 'encar_korea').length;
  ...
  const filteredCars = cars.filter(car => {
    ...
    if (filter === 'korean_import') return car.source === 'korean_import' || car.source === 'encar_korea';
    if (filter === 'hm_local')     return car.source !== 'korean_import' && car.source !== 'encar_korea';
    return true;
  });

  const FILTERS = [
    { key: 'all'           as const, label: 'الكل',  count: cars.length },
    { key: 'korean_import' as const, label: 'كوري', count: koreanCount },
    { key: 'hm_local'      as const, label: 'محلي',  count: localCount  },
  ];
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">
            {loading ? 'جاري التحميل...' : `${cars.length} سيارة إجمالاً · ${koreanCount} كوري · ${localCount} محلي`}
          </p>
        </div>
        <Link
          href="/admin/cars/new"
          className="flex items-center gap-2 px-5 py-3 bg-luxury-gold text-black font-bold rounded-xl hover:bg-white transition-all text-sm shrink-0"
        >
      ...
      {/* ── شريط الاستيراد التلقائي ── */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-gradient-to-br from-luxury-gold/5 to-transparent border border-luxury-gold/15 rounded-2xl p-5">
        <div className="flex items-center gap-3">
          <Globe className="w-4 h-4 text-luxury-gold shrink-0" />
          <div>
            <p className="text-sm font-black text-white">استيراد تلقائي من Encar Korea</p>
            <p className="text-white/30 text-xs mt-0.5">
              يجلب <strong className="text-white/60">{IMPORT_BATCH} سيارة</strong> جديدة مترجمة مع صور وعلامة مائية
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-3 items-center">
        ...
        <div className="flex gap-1 bg-white/5 border border-white/10 rounded-xl p-1 shrink-0">
          {FILTERS.map(f => (
            ...
            >
              {f.label}
              <span className={`mr-1.5 text-[10px] ${filter === f.key ? 'text-black/60' : 'text-white/20'}`}>
                ({loading ? '…' : f.count})
              </span>
            </button>
          ))}
        ...
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="ابحث بالاسم أو الماركة أو الموديل..."
            value={searchTerm}
        ...
        <button
          onClick={fetchCars}
          className="p-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 text-white/40 hover:text-white transition-all shrink-0"
          title="تحديث"
        >
          ...
          <table className="w-full text-right" dir="rtl">
            <thead>
              <tr className="bg-white/5 border-b border-white/10">
                ...
                <th className="px-6 py-4 text-xs font-bold text-white/40 uppercase tracking-widest">النوع</th>
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-20 text-center text-red-400 font-bold">{error}</td>
                </tr>
              ) : filteredCars.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-20 text-center text-white/20 font-bold">
                    لا توجد سيارات مطابقة للبحث
                  </td>
                </tr>
              ) : (
                filteredCars.map(car => {
                  const isKorean = car.source === 'korean_import' || car.source === 'encar_korea';
                  return (
                    <tr key={car._id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <div>
                            <div className="font-bold text-sm line-clamp-1">{formatCarTitle(car.title)}</div>
                            <div className="text-[10px] text-white/30">{car.year}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm font-semibold text-white/60">
                        {cleanKoreanText(car.brand || car.make || '—')}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        {isKorean ? (
                          <span className="px-2.5 py-1 rounded-lg bg-luxury-gold/10 text-luxury-gold text-[10px] font-black">
                            Encar
                          </span>
                        ) : (
                          <span className="px-2.5 py-1 rounded-lg bg-white/5 text-white/40 text-[10px] font-black">
                            محلي
                          </span>
                        )}
                      </td>
