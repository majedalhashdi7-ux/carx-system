...
export default function AdminNewBrandPage() {
  ...
  const [targetShowroom, setTargetShowroom] = useState('hm_local');
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-6 backdrop-blur-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-xs font-bold text-white/40 mb-2 uppercase tracking-widest">اسم الوكالة (بالإنجليزي)</label>
                <input
                  type="text"
                  value={nameEn}
              ...
              {/* Circle Logo Uploader - full width */}
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-6 backdrop-blur-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2">
                <label className="block text-xs font-bold text-white/40 mb-2 uppercase tracking-widest">موقع الوكالة</label>
                <input
                  type="text"
                  value={location}
                  onChange={(e) => setLocation(e.target.value)}
                  placeholder="مثال: طريق الملك عبدالعزيز، الرياض"
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-luxury-gold/40 text-right"
                  dir="rtl"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-luxury-gold/40 font-mono text-left"
                  dir="ltr"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-white/40 mb-2 uppercase tracking-widest">رقم واتساب</label>
                <input
                  type="text"
                  value={whatsapp}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <textarea
                  ...
                  placeholder="اكتب نبذة تعريفية عن الوكالة..."
              ...
              <div>
                <label className="block text-xs font-bold text-white/40 mb-2 uppercase tracking-widest">الوصف بالإنجليزي</label>
                <textarea
                  value={description}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-5 bg-luxury-gold text-black font-black text-lg rounded-2xl flex items-center justify-center gap-2 hover:bg-white transition-all shadow-xl shadow-luxury-gold/10 disabled:opacity-50"
          >
            {loading ? (
