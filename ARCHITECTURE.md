...
يدعم النظام استضافة عدة معارض (Showrooms) على نفس الكود البرمجي مع عزل كامل للبيانات:
- **عزل قاعدة البيانات:** يتم تحديد المعرض بناءً على الدومين (Domain) أو المعرف (Tenant ID)، ويقوم `tenantMiddleware` بتوجيه الطلبات لقاعدة البيانات الخاصة بكل مستأجر.
- **الفهارس المركبة:** جميع النماذج (Models) تستخدم `tenantId` كجزء من الفهرس الفريد لضمان عدم تداخل البيانات (مثل وجود مستخدم `admin` لكل معرض).
- **التهيئة التلقائية:** عند ربط دومين جديد، يقوم النظام تلقائياً بإنشاء الإعدادات وحساب الأدمن الافتراضي للمعرض الجديد.

## 🛡️ مراقبة النظام (Monitoring)
تم دمج نظام **Sentry** للمراقبة اللحظية:
- **Frontend:** تتبع أخطاء المتصفح وتفاعل المستخدمين.
*   **Backend:** تتبع أخطاء الـ API، فشل الاتصال بقاعدة البيانات، وتحليل أداء الاستعلامات.
- **العمل في كافة البيئات:** يعمل النظام محلياً (Local) وعبر الإنترنت (Vercel) بشكل متزامن.

## 🚀 التحديثات الأخيرة (May 2026)
- **واجهة المعرض (Showroom):** تمت إضافة صفحة متقدمة لعرض السيارات `/showroom` في نظام CAR X تتميز بنظام فلاتر معقد (Mark, Fuel, Transmission)، وتدعم تبديل طرق العرض (Grid/List)، وتحتوي على تصميم فاخر.
- **توسعة الـ API Client:** تم دعم الربط المباشر بـ `parts`، `brands`، و `admin/stats` لاستكمال ميزات المنصة.
- **حماية المسارات (Auth Guard):** تم إنشاء `AdminLayout` كـ Client Component للتحقق اللحظي من توفر صلاحيات `admin` عبر `localStorage` و الـ Backend قبل السماح بدخول مسارات `/admin*`.
- **لوحة التحكم الحية:** لوحة تحكم الإدارة `/admin` الآن تقرأ الإحصائيات مباشرة من خادم الواجهة الخلفية بدل عرض أرقام ثابتة.
